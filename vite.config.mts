...
// Dev (watch): rebuilds into a live Obsidian plugins/advanced-infobox/ directory so changes
...
function devOutDir(): string {
  ...
  if (pluginsDir) {
    return path.join(pluginsDir, "advanced-infobox");
  }
  return "test-vault/.obsidian/plugins/advanced-infobox";
}
...
function copyManifest(): Plugin {
  return {
    ...
    closeBundle() {
      ...
      if (isWatch) {
        console.log(`[advanced-infobox] Deployed to ${outDir}`);
      }
    },
  };
}
